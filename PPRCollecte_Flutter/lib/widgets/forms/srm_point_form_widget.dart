...
class _SrmPointFormWidgetState extends State<SrmPointFormWidget>
  ...
  // ── Verrouillage (SPRINT 8) ──
  // true = formulaire verrouillé après synchro (ou anomalie traitée + sync)
  ...
  void initState() {
    ...
    if (widget.existingData != null) {
      ...
      // Calculer l'état de verrouillage une seule fois à l'ouverture
  ...
  Future<void> _save() async {
    // ── SPRINT 8 : garde de sécurité — impossible de sauvegarder si verrouillé ──
    if (_isLocked) return;
  ...
  Widget _buildField(String field) {
    ...
    // ── SPRINT 8 : si verrouillé, tous les champs non-coordonnées sont readOnly ──
    Widget fieldWidget;
    ...
    if (isTypeField) {
      fieldWidget = Padding(
        ...
        child: DropdownButtonFormField<String>(
          ...
          onChanged: (_isObjetIncomplet || _isLocked)
              ? null // désactivé si objet incomplet ou verrouillé
              : (v) => controller.text = v ?? '',
      ...
      // Coordonnées : TOUJOURS readOnly (GPS), même en mode édition libre
      fieldWidget = Padding(
        padding: const EdgeInsets.only(bottom: 12),
        child: TextFormField(
      ...
      // Champ normal : readOnly si verrouillé OU objet incomplet
    ...
    // ── Opacité réduite quand objet incomplet ou verrouillé (sauf coordonnées) ──
  ...
  // ────────────────────────────────────────────
  // Section Anomalie (inchangée sauf verrouillage)
  // ────────────────────────────────────────────
  Widget _buildAnomalieSection() {
    // Désactivée si objet incomplet est activé OU si formulaire verrouillé
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            // Masquer la section Incomplet si verrouillé
